...
import { setResources, removeResource } from '@/store/slices/officeSlice';
import { useResources } from '@/hooks/useResources';
import type { ResResource } from '@/type/resource';
type SortOption = 'price-asc' | 'price-desc' | 'name';
import { useCreateReservation } from '@/hooks/useSubmitInformation';
...
export default function ChooseResources() {
  ...
  const { resources, isPending, isError, error } = useResources();
  ...
  const addToCart = (resource: ResResource) => {
    if (resource) {
  ...
  const { mutate, isSuccess, data } = useCreateReservation();
  const { startDate, endDate, total } = useSelector(
  ...
  const handleConfirmRequest = () => {
    const reservationPayload = {
      ...
      isPrivate: true,
      description: 'TestikngSome notes',
    };

    mutate(reservationPayload);
  };
  useEffect(() => {
    if (isSuccess) {
                    ...
                    <Card key={resource.id} hover padding="md">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          className={`px-4 py-2 rounded-lg font-medium transition-all duration-300 ${
                            // resource.available ?
                            'bg-red-500 text-white hover:bg-red-400 hover:shadow-lg'
                            // : 'bg-gray-200 text-gray-400 cursor-not-allowed'
                          }`}
                        >
                          Remove
